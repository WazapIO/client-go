...
type ApiUploadMediaFromUrlRequest struct {
	ctx context.Context
	ApiService *MessageSendingApiService
	instanceKey string
	type_ *string
...
// Media type
func (r ApiUploadMediaFromUrlRequest) Type_(type_ string) ApiUploadMediaFromUrlRequest {
	r.type_ = &type_
	return r
}

// Media data
func (r ApiUploadMediaFromUrlRequest) Data(data UrlMediaUploadPayload) ApiUploadMediaFromUrlRequest {
	r.data = &data
	return r
}

func (r ApiUploadMediaFromUrlRequest) Execute() (*APIResponse, *http.Response, error) {
	return r.ApiService.UploadMediaFromUrlExecute(r)
}

/*
UploadMediaFromUrl Upload media from url.

Uploads media from a url to WhatsApp servers and returns the media keys.
Store the returned media keys, as you will need them to send media messages

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param instanceKey Instance key
 @return ApiUploadMediaFromUrlRequest
*/
func (a *MessageSendingApiService) UploadMediaFromUrl(ctx context.Context, instanceKey string) ApiUploadMediaFromUrlRequest {
	return ApiUploadMediaFromUrlRequest{
		ApiService: a,
		ctx: ctx,
		instanceKey: instanceKey,
...
// Execute executes the request
//  @return APIResponse
func (a *MessageSendingApiService) UploadMediaFromUrlExecute(r ApiUploadMediaFromUrlRequest) (*APIResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "MessageSendingApiService.UploadMediaFromUrl")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/instances/{instance_key}/send/upload-url"
	localVarPath = strings.Replace(localVarPath, "{"+"instance_key"+"}", url.PathEscape(parameterToString(r.instanceKey, "")), -1)
