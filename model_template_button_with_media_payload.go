/*
WhatsAPI Go

The V2 of WhatsAPI Go

API version: 2.0
Contact: manjit@sponsorbook.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package WhatsAPI

import (
	"encoding/json"
)

// TemplateButtonWithMediaPayload struct for TemplateButtonWithMediaPayload
type TemplateButtonWithMediaPayload struct {
	Buttons []TemplateButton `json:"buttons"`
	ContentText *string `json:"content_text,omitempty"`
	Footer *string `json:"footer,omitempty"`
	Media FileUpload `json:"media"`
	To string `json:"to"`
}

// NewTemplateButtonWithMediaPayload instantiates a new TemplateButtonWithMediaPayload object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTemplateButtonWithMediaPayload(buttons []TemplateButton, media FileUpload, to string) *TemplateButtonWithMediaPayload {
	this := TemplateButtonWithMediaPayload{}
	this.Buttons = buttons
	this.Media = media
	this.To = to
	return &this
}

// NewTemplateButtonWithMediaPayloadWithDefaults instantiates a new TemplateButtonWithMediaPayload object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTemplateButtonWithMediaPayloadWithDefaults() *TemplateButtonWithMediaPayload {
	this := TemplateButtonWithMediaPayload{}
	return &this
}

// GetButtons returns the Buttons field value
func (o *TemplateButtonWithMediaPayload) GetButtons() []TemplateButton {
	if o == nil {
		var ret []TemplateButton
		return ret
	}

	return o.Buttons
}

// GetButtonsOk returns a tuple with the Buttons field value
// and a boolean to check if the value has been set.
func (o *TemplateButtonWithMediaPayload) GetButtonsOk() ([]TemplateButton, bool) {
	if o == nil {
    return nil, false
	}
	return o.Buttons, true
}

// SetButtons sets field value
func (o *TemplateButtonWithMediaPayload) SetButtons(v []TemplateButton) {
	o.Buttons = v
}

// GetContentText returns the ContentText field value if set, zero value otherwise.
func (o *TemplateButtonWithMediaPayload) GetContentText() string {
	if o == nil || isNil(o.ContentText) {
		var ret string
		return ret
	}
	return *o.ContentText
}

// GetContentTextOk returns a tuple with the ContentText field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TemplateButtonWithMediaPayload) GetContentTextOk() (*string, bool) {
	if o == nil || isNil(o.ContentText) {
    return nil, false
	}
	return o.ContentText, true
}

// HasContentText returns a boolean if a field has been set.
func (o *TemplateButtonWithMediaPayload) HasContentText() bool {
	if o != nil && !isNil(o.ContentText) {
		return true
	}

	return false
}

// SetContentText gets a reference to the given string and assigns it to the ContentText field.
func (o *TemplateButtonWithMediaPayload) SetContentText(v string) {
	o.ContentText = &v
}

// GetFooter returns the Footer field value if set, zero value otherwise.
func (o *TemplateButtonWithMediaPayload) GetFooter() string {
	if o == nil || isNil(o.Footer) {
		var ret string
		return ret
	}
	return *o.Footer
}

// GetFooterOk returns a tuple with the Footer field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TemplateButtonWithMediaPayload) GetFooterOk() (*string, bool) {
	if o == nil || isNil(o.Footer) {
    return nil, false
	}
	return o.Footer, true
}

// HasFooter returns a boolean if a field has been set.
func (o *TemplateButtonWithMediaPayload) HasFooter() bool {
	if o != nil && !isNil(o.Footer) {
		return true
	}

	return false
}

// SetFooter gets a reference to the given string and assigns it to the Footer field.
func (o *TemplateButtonWithMediaPayload) SetFooter(v string) {
	o.Footer = &v
}

// GetMedia returns the Media field value
func (o *TemplateButtonWithMediaPayload) GetMedia() FileUpload {
	if o == nil {
		var ret FileUpload
		return ret
	}

	return o.Media
}

// GetMediaOk returns a tuple with the Media field value
// and a boolean to check if the value has been set.
func (o *TemplateButtonWithMediaPayload) GetMediaOk() (*FileUpload, bool) {
	if o == nil {
    return nil, false
	}
	return &o.Media, true
}

// SetMedia sets field value
func (o *TemplateButtonWithMediaPayload) SetMedia(v FileUpload) {
	o.Media = v
}

// GetTo returns the To field value
func (o *TemplateButtonWithMediaPayload) GetTo() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.To
}

// GetToOk returns a tuple with the To field value
// and a boolean to check if the value has been set.
func (o *TemplateButtonWithMediaPayload) GetToOk() (*string, bool) {
	if o == nil {
    return nil, false
	}
	return &o.To, true
}

// SetTo sets field value
func (o *TemplateButtonWithMediaPayload) SetTo(v string) {
	o.To = v
}

func (o TemplateButtonWithMediaPayload) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["buttons"] = o.Buttons
	}
	if !isNil(o.ContentText) {
		toSerialize["content_text"] = o.ContentText
	}
	if !isNil(o.Footer) {
		toSerialize["footer"] = o.Footer
	}
	if true {
		toSerialize["media"] = o.Media
	}
	if true {
		toSerialize["to"] = o.To
	}
	return json.Marshal(toSerialize)
}

type NullableTemplateButtonWithMediaPayload struct {
	value *TemplateButtonWithMediaPayload
	isSet bool
}

func (v NullableTemplateButtonWithMediaPayload) Get() *TemplateButtonWithMediaPayload {
	return v.value
}

func (v *NullableTemplateButtonWithMediaPayload) Set(val *TemplateButtonWithMediaPayload) {
	v.value = val
	v.isSet = true
}

func (v NullableTemplateButtonWithMediaPayload) IsSet() bool {
	return v.isSet
}

func (v *NullableTemplateButtonWithMediaPayload) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTemplateButtonWithMediaPayload(val *TemplateButtonWithMediaPayload) *NullableTemplateButtonWithMediaPayload {
	return &NullableTemplateButtonWithMediaPayload{value: val, isSet: true}
}

func (v NullableTemplateButtonWithMediaPayload) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTemplateButtonWithMediaPayload) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


